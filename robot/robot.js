(function(win, options, undefined){
	...
	var hideKefu = false										//是否隐藏人工客服
		...
		, kefuStartWord = 'sending service requirement';					//
	...
	var _generateHtml = function(source){
		...
		if(_isMobile && _con.mod === 'mini'){
			html.push('<a href="'+ _con.newurl +'" target="_blank" class="'+ cssfix +'mobile-title" style="background-color: '+ _colors.header +';"><span class="'+ cssfix +'icon"></span>Asking the service people &gt;</a>');
			html.push('</div></div>');
			return html.join('');
		};
		...
		html.push('<div class="'+ cssfix +'content">');
			...
			html.push('<li class="'+ cssfix +'slide-btn" style="background-color:'+_colors.border+';"><a href="javascript:void(0);"><span class="'+ cssfix +'icon"></span>'+ (!_con.kefuOnly ? 'Bot' : 'Online service') +'</a></li>');
			if(!!_con.feedBtn.url){
				html.push('<li class="'+ cssfix +'slide-feed" style="background-color:'+_con.feedBtn.color+';"><a href="'+ _con.feedBtn.url +'" target="_blank"><span class="'+ cssfix +'icon"></span>Feedback</a></li>');
			};
			html.push('</ul>');
		//左边区域
		...
		html.push('<a class="'+ cssfix +'art" href="javascript:void(0);" title="changing to…">Service by person</a></div>')
		...
		if(!!_con.showCopyright){
			html.push('<a class="'+ cssfix +'clink" href="http://smartnlp.cn" target="_blank">Powered by SSE</a>');
		};
		html.push('<a class="'+ cssfix +'btn-send" style="background-color:'+ _colors.button +'">Sending</a>');
		html.push('</div>');
		html.push('</div>');
		html.push('</div>');
		...
		if(_con.showLeftPanel){
			...
			if(!hideKefu){
				...
				html.push('<span>Service by person</span>');
				html.push('</a></li>');
			};
	...
	var Robot = function(config){
		this.con = $.extend({
			...
			, 'codeImg': null
			/*机器人欢迎语*/
			, 'welcome': 'Hi， My name is $name'
			/*人工客服欢迎语*/
			, 'kefuWelcome': 'Hi，$name is serving to you!'
			/*机器人名字*/
			, 'nick': 'xiao zhi'
			/*消息发送后文本框是否自动失去焦点*/
			, 'autoblur': false
						...
						con.call('getTime',function(e, result){
							...
							con.call('assignKF', data, function(error, result){
								...
								if (!kfInfo || kfInfo === 'NO_KF' || kfInfo === 'NO_KF_ONLINE') {
									/*未正常联系上客服*/
									_isConnChat = -1;
									_this.log('No service people online now');
									if(!_this.welcomed){
										_this.log('连接到客服机器人');
										_this.welcome();
									}
